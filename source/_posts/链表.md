...
## 链表的定义
链表（Linked List）是一种物流存储单元上非连续、非顺序的存储结构，数据元素的逻辑顺序是通过链表中的指针链接实现的，由一系列节点组成。
每个节点包含两部分：1.存储数据元素的数据域；2.存储下一个节点地址的指针域。
![](https://cdn.jsdelivr.net/gh/qw-null/BlogImages/20210924102803.png)
data 表示节点存放的数据 || next 表示下一个节点指向的内存空间
...
#### 创建链表
一般链表的第一个节点为head，用来表示这是一个链表存储，在创建链表的时候将链表的第一个节点默认设置为head。
```javascript
// 创建链表
function LinkedList() {
